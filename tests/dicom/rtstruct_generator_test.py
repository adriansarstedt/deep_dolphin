...
class DicomStudyParserTest(unittest.TestCase):
    ...
    def test_generate_rt_struct(self):
        ...
        output_comparison = difflib.Differ().compare(
            str(generated_rtstruct).split("\n"), 
            str(verified_rtstruct).split("\n"))

        output_differences = filter((lambda l: l[0] == '+' or l[0] == '-'), output_comparison)
        unexpected_differences = filter(self.is_unexpected_tag_difference, output_differences)
        
        self.assertEqual(list(unexpected_differences), [])
    ...
    def is_expected_tag_difference(self, difference):
        for tag in self.expected_tag_differences:
            if tag in difference:
                return True

    def is_unexpected_tag_difference(self, difference):
        return (not self.is_expected_tag_difference(difference))
